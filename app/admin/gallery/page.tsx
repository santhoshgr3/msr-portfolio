...
import { Upload, Trash2, Eye, EyeOff, ImageIcon, Video, Play } from 'lucide-react';
...
export default function AdminGalleryPage() {
  ...
  const clearUpload = () => {
    if (uploadPreview && uploadType === 'video') URL.revokeObjectURL(uploadPreview);
    setUploadFile(null);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setUploadMsg({ type: 'success', text: `${uploadType === 'video' ? 'Video' : 'Photo'} uploaded!` });
    } catch (e: unknown) {
  ...
  const photos = gallery.filter(g => !g.media_type || g.media_type === 'image');
  const videos = gallery.filter(g => g.media_type === 'video');

  const filtered = gallery.filter(g => {
    if (filterType === 'image' && g.media_type === 'video') return false;
    if (filterType === 'video' && g.media_type !== 'video') return false;
    ...
    <div className="p-8">
      <div className="mb-6">
        ...
        <p className="text-[#6B6B6B] text-sm mt-0.5">
          {photos.length} photos · {videos.length} videos · {gallery.filter(g => g.is_visible).length} visible
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-[#E8E8E8] p-6 mb-6">
        ...
        <div className="flex gap-2 mb-5">
          <button
            onClick={() => { setUploadType('image'); clearUpload(); }}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-all ${
              uploadType === 'image' ? 'bg-[#FF6F00] text-white' : 'bg-[#F5F5F5] text-[#6B6B6B] hover:bg-[#E8E8E8]'
            }`}
          >
            <ImageIcon size={14} /> Upload Photo
          </button>
          <button
            onClick={() => { setUploadType('video'); clearUpload(); }}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-all ${
              uploadType === 'video' ? 'bg-[#FF6F00] text-white' : 'bg-[#F5F5F5] text-[#6B6B6B] hover:bg-[#E8E8E8]'
            }`}
          >
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div
            className={`border-2 border-dashed rounded-2xl p-5 text-center cursor-pointer transition-all ${
              uploadPreview ? 'border-[#FF6F00]' : 'border-[#E8E8E8] hover:border-[#FF6F00]'
            }`}
            onClick={() => uploadType === 'image' ? imageInputRef.current?.click() : videoInputRef.current?.click()}
            ...
            {uploadPreview ? (
              <div className="relative">
                {uploadType === 'image' ? (
                  ...
                  <video src={uploadPreview} className="w-full h-44 object-contain rounded-xl" controls />
                )}
                <button
              ...
              <div className="py-8">
                {uploadType === 'video' ? (
                  <Video size={28} className="text-[#6B6B6B] mx-auto mb-2" />
                ) : (
                ...
                <p className="text-xs text-[#6B6B6B] mt-1">
                  {uploadType === 'video' ? 'MP4, MOV, WEBM — max 200 MB' : 'JPG, PNG, WEBP — max 10 MB'}
                </p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-3">
            <div>
              ...
              <input
                ...
                placeholder={uploadType === 'video' ? 'e.g. Rally Highlights — Bhupalpally 2025' : 'e.g. Free Medical Camp — Bhupalpally 2025'}
            ...
            <div>
              ...
              <textarea
                ...
                rows={2}
                className="w-full border border-[#E8E8E8] rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#FF6F00] resize-none transition-colors"
              />
            </div>
            {uploadMsg && (
              <div className={`text-xs px-3 py-2.5 rounded-xl font-semibold ${
                uploadMsg.type === 'success' ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-600 border border-red-200'
              }`}>
                {uploadMsg.text}
              </div>
            )}
            ...
            >
              {uploadType === 'video' ? <Video size={14} /> : <Upload size={14} />}
              {uploading ? 'Uploading...' : uploadType === 'video' ? 'Upload Video' : 'Upload Photo'}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-[#E8E8E8] p-6">
        <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
          <h2 className="font-bold text-[#0D0D0D]">Manage Media ({filtered.length})</h2>
                ...
                <div key={item.id} className={`border rounded-xl overflow-hidden ${item.is_visible ? 'border-[#E8E8E8]' : 'border-dashed border-[#E8E8E8] opacity-60'}`}>
                  ...
                  <div className="p-2.5">
                    <div className="flex items-center gap-1 mb-1">
                      ...
                      {isVideo && (
                        <span className="text-[9px] font-bold bg-[#FF6F00]/10 text-[#FF6F00] px-1.5 py-0.5 rounded flex items-center gap-0.5">
                          <Play size={7} /> VIDEO
                        </span>
                      )}
                    </div>
                    <p className="font-semibold text-[#0D0D0D] text-xs leading-tight truncate">{item.title}</p>
